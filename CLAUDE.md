...
motionless lift ON THE SHEET is the pane's click-to-full-screen. **A finger
or a pen is only WATCHED** — no capture, no preventDefault — because the
browser scrolls the sheet natively under `touch-action: pan-x pan-y` and
announces that with a pointercancel, which stands the press down; a lift
that never moved is the tap that opens full screen (the first cut took only
the mouse and silently killed the finger tap — the probe caught it). Rules
paid for: only a press on the plan's canvases, the sheet wrapper or the bare
...
`scratchpad/panepan-probe.mjs` (19 checks — exact travel, no full screen on
a drag, the click still opens it, the same drag in full screen, a pin still
places, the studio's Pan tool, and under CDP touch: a finger drag scrolls
on both axes, a finger tap opens full screen, two fingers still pinch). Its
trap: Chromium axis-LOCKS a synthetic touch drag that leans to one axis, so
a diagonal finger stroke reports dy 0 — that is the browser, not the pane;
