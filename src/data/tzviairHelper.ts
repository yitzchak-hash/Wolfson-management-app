...
export function windowsOpener(driveRoot: string): string {
  ...
  return [
    ...
    '  # and the app may not know the shared drive\'s own name at all (Shared drives\<drive>\...).',
    '  # Try every top folder under the root with the rest of the path, then every drive',
    '  # folder one level down with the same rest, then with the rest minus its own first part.',
    '  $rest = $p.Substring($root.Length).TrimStart("\\")',
    '  $parts = $rest.Split("\\")',
    '  if ($parts.Length -ge 2) {',
    '    $tail = ($parts | Select-Object -Skip 1) -join "\\"',
    '    $tail2 = if ($parts.Length -ge 3) { ($parts | Select-Object -Skip 2) -join "\\" } else { "" }',
    '    $found = $null',
    ...
    '        $c2 = Join-Path $s.FullName $tail',
    '        if (Test-Path -LiteralPath $c2) { $found = $c2; break }',
    '        if ($tail2 -ne "") { $c3 = Join-Path $s.FullName $tail2; if (Test-Path -LiteralPath $c3) { $found = $c3; break } }',
    '      }',
    '      if ($found) { break }',
    '    }',
    '    if ($found) { $p = $found }',
...
export function macInstaller(driveRoot: string): string {
  ...
  return [
    ...
    '',
    'if [ ! -d "$FOLDER" ]; then',
    '  #  The app names the top folder "Shared drives" and may not know the drive\'s own name;',
    '  #  a localised Mac calls the folder something else. Try every top folder, then every',
    '  #  drive folder one level down, with the rest of the path (and the rest minus its first part).',
    '  REST="${FOLDER#"$ROOT"/}"',
    '  TAIL="${REST#*/}"',
    '  TAIL2="${TAIL#*/}"',
    '  for d in "$ROOT"/*/; do',
    '    [ -d "$d$TAIL" ] && FOLDER="$d$TAIL" && break',
    '    for s in "$d"*/; do',
    '      [ -d "$s$TAIL" ] && FOLDER="$s$TAIL" && break 2',
    '      [ "$TAIL2" != "$TAIL" ] && [ -d "$s$TAIL2" ] && FOLDER="$s$TAIL2" && break 2',
    '    done',
    '  done',
    'fi',
    'if [ ! -d "$FOLDER" ]; then',
