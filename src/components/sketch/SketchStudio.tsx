/**
 * THE SKETCH STUDIO (B7) — the management app's PlanAnnotator, forked.
 *
 * `../plans/PlanAnnotator.tsx` is the byte-for-byte copy and stays untouched
 * (the ground rule); this file began as the same bytes and carries the
 * sketcher's additions on top: the block shelf down the side, the block layer
 * over the sheet, the settings window, the two-tap scale. Every addition is
 * marked `B7`, so the copy-back to the management app (B10) can lift exactly
 * those pieces. Everything else in here is the studio, unchanged.
 */
...
import '../plans/pdfCompat';   // must come before pdf.js
import * as pdfjs from 'pdfjs-dist';
import workerUrl from 'pdfjs-dist/build/pdf.worker.min.mjs?url';
import { useStore } from '../../data/store';
import { useSketchStore } from '../../data/sketcherStore';   // B10: the sketcher's records, inside the management app
...
import { PlanTab, PlanTabsStrip, mintTab, loadTabState, saveTabState } from '../plans/PlanTabs';
import { PenStroke, PenSample, NibWatch, samplesOf, simplify, nearSegment } from '../plans/penInput';
import { TOOLS, toolById, INK_COLORS, HIGHLIGHT_COLORS, rememberColor } from '../plans/annotTools';
import { InkPicker } from '../plans/InkPicker';
import { notePointer, isPalm, isPen, touchWasPalm } from '../../data/pencil';
import { useMarkupScale } from '../../data/markupScale';
import { paintStroke, bubbleTextBox, REF, LINE } from '../plans/paintStroke';
import { PlanPicker } from '../plans/PlanPicker';
import { usePhone } from '../../data/usePhone';
import { useT } from '../../data/strings';
import { applyRules, loadCatalog, sortBlocks, type CatalogBlock } from '../../data/catalog';
import { ASSUMED_M_PER_W, mPerWFromPlan, newItem, nextSystemLetter, optionOf, optionsOf, unitTags, SU as SHEET_SU, type SketchItem } from '../../data/sketchItems';
import { BlockLayer, type BlockLayerHandle } from './BlockLayer';
import { BlockShelf, BlockThumb } from './BlockShelf';
import { BlockSettings } from './BlockSettings';
import { GvsLayer, type SketchTool } from './GvsLayer';
import { SketchTools } from './SketchTools';
import { GvsSettings, PipeSettings } from './SketchSettings';
import { colorForDrop, mergeInto, type Pt, type SketchShape } from '../../data/gvs';
import { type PipeKind, type SketchPipe } from '../../data/pipes';
import { defaultMinDrop } from '../../data/catalog';
import { ExportDialog } from './ExportDialog';
import type { SketchExport } from '../../data/sketchItems';
...
function PlanEditor({
  ...
  /* ── B7: the blocks ─────────────────────────────────────────────────── */
  const t7 = useT();
  const isRtl7 = !!s.isRtl;
  const items7 = useSketchStore(st => st.items);
  const addItem = useSketchStore(st => st.addItem);
  const updateItem = useSketchStore(st => st.updateItem);
  const deleteItem = useSketchStore(st => st.deleteItem);
  const sheets7 = useSketchStore(st => st.sheets);
  const setSheet = useSketchStore(st => st.setSheet);
  const customBlocks = useSketchStore(st => st.blocks);
  const addBlock = useSketchStore(st => st.addBlock);
  const deleteBlock = useSketchStore(st => st.deleteBlock);
  const blockRules = useSketchStore(st => st.blockRules);
  const plans7 = useSketchStore(st => st.plans);
  const [catalogBase, setCatalogBase] = useState<CatalogBlock[]>([]);
  const [catalogErr, setCatalogErr] = useState(false);
  useEffect(() => { let live = true; loadCatalog().then(b => { if (live) setCatalogBase(b); }).catch(() => { if (live) setCatalogErr(true); }); return () => { live = false; }; }, []);
  /** Every block that can be DRAWN — hidden ones included, because a placed record still has to draw. */
  const blocksById = useMemo(() => new Map([...catalogBase, ...customBlocks].map(b => [b.id, b] as const)), [catalogBase, customBlocks]);
  /** What the shelf SELLS: rules applied, hidden dropped, sorted. */
  const shelfBlocks = useMemo(() => sortBlocks(applyRules([...catalogBase, ...customBlocks], blockRules), blockRules), [catalogBase, customBlocks, blockRules]);
  const sheetItems = useMemo(() => items7.filter(i => i.sheetId === planFileId), [items7, planFileId]);
  const sheetRec = sheets7.find(x => x.id === planFileId);
  const planRec = plans7.find(x => x.cleanFileId === planFileId);
  /** Metres across the sheet: the two taps here, else the clean base's own record (B5), else an honest assumption. */
  const mPerW = sheetRec?.mPerW ?? (planRec?.scale ? mPerWFromPlan(planRec.scale.mmPerPx, planRec.width) : ASSUMED_M_PER_W);
  const scaleKnown = sheetRec?.mPerW != null || !!planRec?.scale;
  const systems7 = sheetRec?.systems ?? ['A'];
  // B8: which OPTION of the floor is being drawn on (pick 27) — every record below is filtered by it
  const options8 = useMemo(() => optionsOf(sheetRec), [sheetRec]);
  const [activeOpt, setActiveOpt] = useState<string>(options8[0].id);
  const optId = options8.some(o => o.id === activeOpt) ? activeOpt : options8[0].id;
  const inOpt = useCallback((r: { option?: string }) => optionOf(r, options8) === optId, [options8, optId]);
  const optItems = useMemo(() => sheetItems.filter(inOpt), [sheetItems, inOpt]);
  const pageItems = useMemo(() => optItems.filter(i => i.page === page), [optItems, page]);
  const tags7 = useMemo(() => unitTags(optItems), [optItems]);
  const [system7, setSystem7] = useState('A');
  const [stamp, setStamp] = useState<CatalogBlock | null>(null);
  const [selItem, setSelItem] = useState<string | null>(null);
  const [openItem, setOpenItem] = useState<string | null>(null);
  const [measure, setMeasure] = useState<{ x: number; y: number }[] | null>(null);
  const [measureM, setMeasureM] = useState('');
  const [shelfDrag, setShelfDrag] = useState<{ b: CatalogBlock; x: number; y: number } | null>(null);
  const [shelfOpen, setShelfOpen] = useState(true);
  const layerRef = useRef<BlockLayerHandle>(null);
  const SHELF_W = 126;
  const placeBlock = useCallback((b: CatalogBlock, at: { x: number; y: number; rot: number }, then?: 'select') => {
    const seq = Math.max(0, ...sheetItems.map(i => i.seq)) + 1;
    const it = newItem(b, at, mPerW, { sheetId: planFileId, page, jobId: apartmentId.startsWith('sk-') ? undefined : apartmentId, system: system7, seq, by: authorName, option: optId });
    it.rot = at.rot;
    addItem(it);
    if (then === 'select') { setSelItem(it.id); setTool('move'); }
    return it;
  }, [sheetItems, mPerW, planFileId, page, apartmentId, system7, authorName, addItem, optId]);
  /** A shelf drag let go over the sheet lands ONE block, snapped like any other (pick 18 · 42). */
  const shelfDrop = useCallback((b: CatalogBlock, clientX: number, clientY: number) => {
    setShelfDrag(null);
    const at = layerRef.current?.suAtClient(clientX, clientY);
    if (!at) return;
    placeBlock(b, layerRef.current!.settle(b, at), 'select');
  }, [placeBlock]);
  const renumberItem = useCallback((id: string, n: number) => {
    const it = optItems.find(i => i.id === id); if (!it) return;
    const sys = it.system || 'A';
    const list = optItems.filter(i => i.unit && i.id !== id && (i.system || 'A') === sys).sort((a, b) => a.seq - b.seq);
    list.splice(Math.max(0, Math.min(list.length, n - 1)), 0, it);
    list.forEach((x, i) => { if (x.seq !== i + 1) updateItem(x.id, { seq: i + 1 }); });
  }, [optItems, updateItem]);
  const swapModel = useCallback((id: string, b: CatalogBlock) => {
    const it = sheetItems.find(i => i.id === id); if (!it) return;
    updateItem(id, { blockId: b.id, name: b.name, model: b.model, brand: b.brand, cat: b.cat, unit: b.unit, wCm: it.locked ? b.wCm : it.wCm, hCm: it.locked ? b.hCm : it.hCm, system: b.unit ? (it.system ?? system7) : undefined });
  }, [sheetItems, updateItem, system7]);
  const setScaleFromTaps = useCallback(() => {
    const m = Number(measureM);
    if (!measure || measure.length < 2 || !(m > 0)) return;
    const d = Math.hypot(measure[1].x - measure[0].x, measure[1].y - measure[0].y);
    if (d < 1) return;
    setSheet(planFileId, { mPerW: +((m * SHEET_SU) / d).toFixed(3) });
    setMeasure(null); setMeasureM('');
  }, [measure, measureM, setSheet, planFileId]);
  const scaleWords = scaleKnown ? t7.scaleAcross(mPerW.toFixed(1)) : t7.scaleAssumed;
  /* ── end B7 state ── */

  /* ── B8: the gvs, pipes and ducts, options ───────────────────────────── */
  const shapes8 = useSketchStore(st => st.shapes);
  const addShape = useSketchStore(st => st.addShape);
  const updateShape = useSketchStore(st => st.updateShape);
  const deleteShape = useSketchStore(st => st.deleteShape);
  const pipes8 = useSketchStore(st => st.pipes);
  const addPipe = useSketchStore(st => st.addPipe);
  const updatePipe = useSketchStore(st => st.updatePipe);
  const deletePipe = useSketchStore(st => st.deletePipe);
  const pipeRules = useSketchStore(st => st.pipeRules);
  const [sketchTool, setSketchToolRaw] = useState<SketchTool>(null);
  const [pipeKind, setPipeKind] = useState<PipeKind>('refrigerant');
  const [drop8, setDrop8] = useState(35);
  const [selSk, setSelSk] = useState<{ kind: 'shape' | 'pipe'; id: string } | null>(null);
  const [openSk, setOpenSk] = useState<{ kind: 'shape' | 'pipe'; id: string } | null>(null);
  const [compare, setCompare] = useState(false);
  const setSketchTool = useCallback((t: SketchTool) => { setSketchToolRaw(t); if (t) { setStamp(null); setMeasure(null); setSelItem(null); setSelSk(null); setPicked(null); } }, []);
  const onSheet = useCallback(<T extends { sheetId: string; page: number }>(r: T) => r.sheetId === planFileId && r.page === page, [planFileId, page]);
  const optShapes = useMemo(() => shapes8.filter(sh => onSheet(sh) && inOpt(sh)), [shapes8, onSheet, inOpt]);
  const optPipes = useMemo(() => pipes8.filter(pp => onSheet(pp) && inOpt(pp)), [pipes8, onSheet, inOpt]);
  const ghostOpt = options8.find(o => o.id !== optId);
  const ghost8 = useMemo(() => {
    if (!compare || !ghostOpt) return null;
    const of = (r: { option?: string }) => optionOf(r, options8) === ghostOpt.id;
    return { shapes: shapes8.filter(sh => onSheet(sh) && of(sh)), pipes: pipes8.filter(pp => onSheet(pp) && of(pp)), items: sheetItems.filter(it => it.page === page && of(it)) };
  }, [compare, ghostOpt, options8, shapes8, pipes8, sheetItems, page, onSheet]);
  const minDropOf = useCallback((it: SketchItem) => blockRules[it.blockId]?.minDrop ?? defaultMinDrop(it.sub, it.cat), [blockRules]);
  const mint = (prefix: string) => `${prefix}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 7)}`;
  /** A fresh box joins any box of the same drop it touches (pick 62). */
  const addGvs = useCallback((pts: Pt[]) => {
    const color = colorForDrop(drop8);
    const m = mergeInto(pts, drop8, color, optShapes);
    m.absorbed.forEach(id => deleteShape(id));
    const sh: SketchShape = { id: mint('sg'), sheetId: planFileId, page, option: optId, pts: m.pts, drop: drop8, color, createdAt: new Date().toISOString(), createdBy: authorName };
    addShape(sh);
    setSelSk({ kind: 'shape', id: sh.id });
    if (m.absorbed.length) onToast?.(t7.gvsMerged);
  }, [drop8, optShapes, deleteShape, planFileId, page, optId, authorName, addShape, onToast, t7]);
  /** A recoloured box may now touch a box of its new drop — it joins it. */
  const changeGvs = useCallback((id: string, patch: Partial<SketchShape>) => {
    const cur = optShapes.find(x => x.id === id); if (!cur) return;
    const next = { ...cur, ...patch };
    if (patch.drop != null || patch.color != null) {
      const m = mergeInto(next.pts, next.drop, next.color, optShapes.filter(x => x.id !== id));
      m.absorbed.forEach(x => deleteShape(x));
      updateShape(id, { ...patch, pts: m.pts });
      if (m.absorbed.length) onToast?.(t7.gvsMerged);
    } else updateShape(id, patch);
  }, [optShapes, deleteShape, updateShape, onToast, t7]);
  const addPipe8 = useCallback((pp: { kind: PipeKind; pts: Pt[]; fromId?: string; toId?: string; widthCm?: number }) => {
    const rec: SketchPipe = { id: mint('sp'), sheetId: planFileId, page, option: optId, ...pp, createdAt: new Date().toISOString(), createdBy: authorName };
    addPipe(rec);
  }, [planFileId, page, optId, authorName, addPipe]);
  /** Options (picks 63–67): + asks copy-or-blank; a copy carries everything drawn on the floor. */
  const addOption = useCallback((copy: boolean) => {
    const id = mint('o');
    const name = t7.optionName(options8.length + 1);
    setSheet(planFileId, { options: [...options8, { id, name }] });
    if (copy) {
      for (const it of sheetItems.filter(inOpt)) addItem({ ...it, id: mint('si'), option: id });
      for (const sh of shapes8.filter(x => x.sheetId === planFileId && inOpt(x))) addShape({ ...sh, id: mint('sg'), option: id });
      for (const pp of pipes8.filter(x => x.sheetId === planFileId && inOpt(x))) addPipe({ ...pp, id: mint('sp'), option: id });
    }
    setActiveOpt(id); setSelSk(null); setSelItem(null);
  }, [t7, options8, setSheet, planFileId, sheetItems, inOpt, addItem, shapes8, addShape, pipes8, addPipe]);
  const wipeOption = useCallback((id: string) => {
    const of = (r: { option?: string }) => optionOf(r, options8) === id;
    sheetItems.filter(of).forEach(it => deleteItem(it.id));
    shapes8.filter(x => x.sheetId === planFileId && of(x)).forEach(x => deleteShape(x.id));
    pipes8.filter(x => x.sheetId === planFileId && of(x)).forEach(x => deletePipe(x.id));
  }, [options8, sheetItems, deleteItem, shapes8, planFileId, deleteShape, pipes8, deletePipe]);
  const deleteOption = useCallback((id: string) => {
    if (options8.length < 2) return;
    wipeOption(id);
    const rest = options8.filter(o => o.id !== id);
    setSheet(planFileId, { options: rest });
    setActiveOpt(rest[0].id); setSelSk(null); setSelItem(null);
  }, [options8, wipeOption, setSheet, planFileId]);
  /* ── end B8 state ── */

  /* ── B9: output ── */
  const [exporting, setExporting] = useState(false);
  const addExport = useSketchStore(st => st.addExport);
  // B10: in the management app the job IS the apartment — its family name, address and phone feed the title block
  const apts9 = useStore(st => st.apartments);
  const projectId9 = useStore(st => st.currentProjectId);
  const job9 = useMemo(() => { const a = apts9.find(x => x.id === apartmentId); return a ? { id: a.id, projectId: projectId9, label: aptLabel(a), address: a.address ?? '', phone: a.phone ?? '' } : undefined; }, [apts9, apartmentId, projectId9]);
  const recordsFor = useCallback((id: string) => {
    const of = (r: { option?: string }) => optionOf(r, options8) === id;
    return { items: sheetItems.filter(it => it.page === page && of(it)), shapes: shapes8.filter(x => onSheet(x) && of(x)), pipes: pipes8.filter(x => onSheet(x) && of(x)) };
  }, [options8, sheetItems, page, shapes8, pipes8, onSheet]);
  const exportVersion = (sheetRec?.exportCount ?? 0) + 1;
  const finishExport = useCallback((out: { options: string[]; files: { name: string; fileId?: string; url?: string }[] }) => {
    const rec: SketchExport = { id: `sx-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 7)}`, sheetId: planFileId, jobId: job9?.id, projectId: job9?.projectId, version: exportVersion, options: out.options, files: out.files, createdAt: new Date().toISOString(), createdBy: authorName };
    addExport(rec);
    setSheet(planFileId, { exportCount: exportVersion });
  }, [planFileId, job9, exportVersion, authorName, addExport, setSheet]);
  /* ── end B9 ── */
  ...
  useEffect(() => {
    function key(e: KeyboardEvent) {
      ...
      // B7: Delete takes the picked block; Escape backs out of the blocks first
      if ((e.key === 'Delete' || e.key === 'Backspace') && selSk && !textDraft && !openSk && !openItem && tool === 'move') {
        const tag = (e.target as HTMLElement | null)?.tagName;
        if (tag !== 'INPUT' && tag !== 'TEXTAREA') { if (selSk.kind === 'shape') deleteShape(selSk.id); else deletePipe(selSk.id); setSelSk(null); e.preventDefault(); e.stopPropagation(); return; }
      }
      if ((e.key === 'Delete' || e.key === 'Backspace') && selItem && !textDraft && !openItem && tool === 'move') {
        const tag = (e.target as HTMLElement | null)?.tagName;
        if (tag !== 'INPUT' && tag !== 'TEXTAREA') { deleteItem(selItem); setSelItem(null); e.preventDefault(); e.stopPropagation(); return; }
      }
      if (e.key === 'Escape') {
        ...
        else if (openSk) setOpenSk(null);
        else if (sketchTool) setSketchTool(null);
        else if (openItem) setOpenItem(null);
        else if (stamp) setStamp(null);
        else if (measure) { setMeasure(null); setMeasureM(''); }
        else if (shelfDrag) setShelfDrag(null);
        ...
        else if (selItem) setSelItem(null);
        else if (selSk) setSelSk(null);
      ...
      if (!readOnly && !e.ctrlKey && !e.metaKey && !openItem && !measure && !openSk && !sketchTool && !exporting) {
    ...
    >
      ...
      */}
      {(() => {
        ...
        */}
        ...
        {!compact && (<>
          ...
          {/* B9: the sheets */}
          {!locked && (
            <button data-sketch-export onClick={() => setExporting(true)} title={t7.exportWindowTitle}
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[12px] font-bold text-white" style={{ backgroundColor: ACCENT }}>
              <FileDown size={14} /> {t7.exportBtn}
            </button>
          )}
      ...
      <div className="flex-1 min-h-0 flex">
          ...
          <div ref={stageRef}
            ...
            */
            ...
            {loadErr ? (
              ...
              <div className="relative shadow-2xl m-auto" style={{ backgroundColor: '#fff' }} data-sheet
                onPointerDown={e => {
                  const el = e.target as Element;
                  if (selItem && !el.closest?.('[data-item],[data-item-chrome]')) setSelItem(null);
                  if (selSk && !el.closest?.('[data-gvs],[data-pipe],[data-drop-chips],[data-gvs-warn],[data-item],[data-item-chrome]')) setSelSk(null);
                }}>
                {/*
                ...
                */}
                ...
                />
                {/* B8: lowered ceilings, pipes and ducts — UNDER the blocks */}
                {nat && (
                  <GvsLayer
                    shapes={optShapes}
                    pipes={optPipes}
                    ...
                    mPerW={mPerW}
                    aspect={nat.h / nat.w}
                    tool={locked ? null : sketchTool}
                    pipeKind={pipeKind}
                    pipeRules={pipeRules}
                    active={!locked && tool === 'move'}
                    selected={selSk}
                    onSelect={sel => { setSelSk(sel); if (sel) { setSelItem(null); setPicked(null); } }}
                    onAddShape={addGvs}
                    onAddPipe={addPipe8}
                    onChangeShape={changeGvs}
                    onOpenShape={id => setOpenSk({ kind: 'shape', id })}
                    onOpenPipe={id => setOpenSk({ kind: 'pipe', id })}
                    ghost={ghost8}
                    pdfCanvas={() => pdfRef.current}
                    readOnly={locked}
                    words={{ needs: t7.gvsWarn, fix: t7.gvsFix, m2: 'm²', runM: 'm' }}
                  />
                )}
                {/* B7: the blocks, over the ink (their own pointer rules are inside) */}
                {nat && (
                  <BlockLayer
                    ref={layerRef}
                    items={pageItems}
                    ...
                    mPerW={mPerW}
                    aspect={nat.h / nat.w}
                    active={!locked && tool === 'move' && !sketchTool}
                    stamp={locked ? null : stamp}
                    measuring={measure}
                    onMeasureTap={p => setMeasure(m => (!m || m.length >= 2 ? [p] : [...m, p]))}
                    selected={selItem}
                    onSelect={id => { setSelItem(id); if (id) { setPicked(null); setSelSk(null); } }}
                    onPlace={(b, at) => { placeBlock(b, at); }}
                    onChange={(id, patch) => updateItem(id, patch)}
                    onOpen={id => setOpenItem(id)}
                    tags={tags7}
                    pdfCanvas={() => pdfRef.current}
                    readOnly={locked}
                    words={{ hideLabel: t7.hideLabel, showLabel: t7.showLabel, flipLabel: t7.flipLabel, locked: t7.locked, unlocked: t7.unlocked, turn: t7.turn, resize: t7.resize, stretch: t7.stretch }}
                  />
                )}
                {textDraft && (() => {
        ...
        {/* B7: the shelf, always open down the far side (pick 41); on a phone it folds behind a button */}
        {!locked && (!compact || shelfOpen) && (
          <BlockShelf
            blocks={shelfBlocks}
            items={optItems}
            top={(
              <SketchTools
                tool={sketchTool} onTool={setSketchTool}
                pipeKind={pipeKind} onPipeKind={setPipeKind} pipeRules={pipeRules}
                drop={drop8} onDrop={setDrop8}
                options={options8} active={optId}
                onPick={id => { setActiveOpt(id); setSelSk(null); setSelItem(null); }}
                onAdd={addOption}
                onRename={(id, name) => setSheet(planFileId, { options: options8.map(o => (o.id === id ? { ...o, name } : o)) })}
                onDelete={deleteOption}
                onClear={() => wipeOption(optId)}
                compare={compare} onCompare={setCompare}
              />
            )}
            systems={systems7}
            system={systems7.includes(system7) ? system7 : systems7[0]}
            onSystem={setSystem7}
            onAddSystem={() => { const l = nextSystemLetter(systems7); setSheet(planFileId, { systems: [...systems7, l] }); setSystem7(l); }}
            stamp={stamp}
            onStamp={b => { setStamp(b); setMeasure(null); if (b) setSelItem(null); }}
            onDragMove={(b, x, y) => setShelfDrag({ b, x, y })}
            onDragEnd={shelfDrop}
            scaleWords={catalogErr ? t7.catalogFailed : scaleWords}
            scaleKnown={scaleKnown}
            measuring={!!measure}
            onMeasure={() => { setStamp(null); setMeasure(m => (m ? null : [])); setMeasureM(''); }}
            onAddPng={b => addBlock(b)}
            onDeletePng={id => deleteBlock(id)}
            width={compact ? Math.min(SHELF_W, 160) : SHELF_W}
            padBottom={compact ? 44 : 0}
            isRtl={isRtl7}
          />
        )}
      </div>
      {!locked && compact && (
        <button data-shelf-toggle onClick={() => setShelfOpen(o => !o)} className="fixed z-[160] rounded-full px-3 h-9 text-[12px] font-bold text-white shadow-lg"
          style={{ right: 10, bottom: railRow ? 74 : 12, backgroundColor: shelfOpen ? ACCENT : NAVY }}>{shelfOpen ? t7.done : t7.blocksWord}</button>
      )}
      {/* B7: the block riding a shelf drag */}
      {shelfDrag && (
        <div data-shelf-ghost className="fixed z-[200] pointer-events-none rounded-lg bg-white/90 shadow-xl p-1 flex items-center justify-center" style={{ left: shelfDrag.x - 30, top: shelfDrag.y - 20, width: 60, height: 40 }}>
          <BlockThumb b={shelfDrag.b} size={54} />
        </div>
      )}
      {/* B7: the two taps' question */}
      {measure && measure.length === 2 && (
        <div className="fixed inset-0 z-[188] flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(9,14,22,.45)' }} onClick={() => { setMeasure(null); setMeasureM(''); }}>
          <form data-measure-ask className="bg-white text-gray-900 rounded-2xl shadow-2xl p-4 w-full max-w-xs flex flex-col gap-2" dir={isRtl7 ? 'rtl' : 'ltr'} onClick={e => e.stopPropagation()} onSubmit={e => { e.preventDefault(); setScaleFromTaps(); }}>
            <div className="text-[13px] font-extrabold">{t7.measureMetres}</div>
            <input data-measure-input autoFocus type="number" step="0.01" min="0.01" value={measureM} onChange={e => setMeasureM(e.target.value)} className="border rounded-lg h-10 px-2 text-[14px] tabular-nums" />
            <div className="flex gap-2">
              <button type="submit" data-measure-set disabled={!(Number(measureM) > 0)} className="flex-1 h-9 rounded-xl text-white font-bold text-[12.5px] disabled:opacity-40" style={{ backgroundColor: NAVY }}>{t7.measureSet}</button>
              <button type="button" onClick={() => { setMeasure(null); setMeasureM(''); }} className="h-9 px-3 rounded-xl text-[12.5px] font-bold" style={{ backgroundColor: '#f1f5f9' }}>{t7.cancel}</button>
            </div>
          </form>
        </div>
      )}
      ...
      {exporting && nat && (
        <ExportDialog
          ...
          pipeRules={pipeRules}
          aspect={nat.h / nat.w}
          canvases={() => ({ pdf: pdfRef.current, ink: inkRef.current })}
          titleDefaults={{
            family: job9?.label ?? planRec?.title?.project ?? '',
            phone: job9?.phone ?? '',
            address: job9?.address ?? '',
            floor: planRec?.floor ?? '',
            project: planRec?.title?.project && job9 ? planRec.title.project : '',
            planType: t7.planTypeAc,
            drawnBy: authorName,
          }}
          version={exportVersion}
          jobFolderId={driveFolderUrl ? extractFolderId(driveFolderUrl) ?? undefined : undefined}
          driveConfigured={isUploadBackendConfigured()}
          onDone={finishExport}
          onClose={() => setExporting(false)}
          isRtl={isRtl7}
        />
      )}
      {/* B8: a box's or a run's settings (pick 52) */}
      {openSk?.kind === 'shape' && (() => {
        const sh = optShapes.find(x => x.id === openSk.id); if (!sh) return null;
        return <GvsSettings shape={sh} mPerW={mPerW} onChange={patch => changeGvs(sh.id, patch)} onDelete={() => { deleteShape(sh.id); setSelSk(null); }} onClose={() => setOpenSk(null)} isRtl={isRtl7} />;
      })()}
      {openSk?.kind === 'pipe' && (() => {
        const pp = optPipes.find(x => x.id === openSk.id); if (!pp) return null;
        return <PipeSettings pipe={pp} items={pageItems} tags={tags7} mPerW={mPerW} onChange={patch => updatePipe(pp.id, patch)} onDelete={() => { deletePipe(pp.id); setSelSk(null); }} onClose={() => setOpenSk(null)} isRtl={isRtl7} />;
      })()}
      {/* B7: a block's settings (picks 48 · 52) */}
      {openItem && (() => {
        const it = sheetItems.find(i => i.id === openItem);
        if (!it) return null;
        return (
          <BlockSettings
            item={it}
            block={blocksById.get(it.blockId)}
            catalog={shelfBlocks}
            items={optItems}
            systems={systems7}
            tag={tags7.get(it.id)}
            onChange={patch => updateItem(it.id, patch)}
            onRenumber={n => renumberItem(it.id, n)}
            onSwapModel={b => swapModel(it.id, b)}
            onDelete={() => { deleteItem(it.id); setSelItem(null); }}
      ...
      {showMore && (
        <>
          ...
          <div data-plan-more-sheet className="fixed z-[159] rounded-2xl overflow-hidden bg-white p-1.5"
            ...
            {/* B9: the sheets — the phone bar has no room for the button, so the door is here */}
            {!locked && (
              <SheetRow icon={FileDown} label={t7.exportBtn} hook="sketch-export"
                hint={t7.exportWindowTitle}
                onClick={() => { setShowMore(false); setExporting(true); }} />
            )}
 ...
 */
function SheetRow({ icon: Icon, label, hint, danger, disabled, onClick, hook }: {
  ...
  disabled?: boolean;
  onClick: () => void;
  /** B10: a harness hook (`data-hook`), so a row can be found by what it is rather than by its words. */
  hook?: string;
}) {
  return (
    <button onClick={onClick} disabled={disabled} data-hook={hook}
...
export function SketchStudio(props: PlanHostProps) {
  const {
