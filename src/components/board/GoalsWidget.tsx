 ...
 *  - view defaults by SURFACE: the dashboard's copy opens as the compact
 *    summary strip, a board copy as the full tile grid — overridable in the
 *    pencil, so nobody has to know the difference to get the right one.
 */
...
type MountHandle = { iframe: HTMLIFrameElement; destroy: () => void };
type WidgetApi = { mount: (target: Element, options: Record<string, unknown>) => MountHandle };
...
function loadWidgetScript(): Promise<WidgetApi> {
...
export function GoalsWidget({ el, c }: { el: CanvasElement; c: WidgetCtx }) {
  ...
  const view = (data.view === 'board' || data.view === 'dashboard')
    ? (data.view as 'board' | 'dashboard')
    : (el.board === DASHBOARD_BOARD ? 'dashboard' : 'board');
  ...
  const interactive = (data.interactive === '1'
    || (data.interactive !== '0' && el.board !== DASHBOARD_BOARD)) && !c.readOnly;
  ...
  }, [sample, view, lang, interactive, max, retryTick]);
  ...
  return (
    <Frame title={String(data.title || 'Goals')} icon={Target} tone="#1e3a5f">
      <div data-goals-widget className="min-h-full">
        {state && (
        ...
        {failed ? (
          ...
          <div ref={slotRef} data-goals-slot />
        )}
      </div>
    </Frame>
  );
