...
- ☑ Wolfson stages split into separate bubbles on a grey→yellow→orange→green ramp: `STAGE_SPLITS` in `stageSplit.ts`, `splitCombinedStages` run by the standing migration — three combined global stages retired, eight children under fixed ids, marks copied, tasks/notes/tipus sets re-pointed (owner: "go on the split", 2026-09-22)
...
- ☑ `scratchpad/cloudclean.mjs`: the 4,095 records found sitting in the wrong workspace's collection, backed up and REMOVED on the owner's word (2026-09-22, 4,095 documents; Wolfson 259 · Job Board 3,520 · Netiv 90 remain)
...
- ☑ The weekly notebook's bars and cards lead with the UNIT (`aptLabel`), the workspace a small tag on the second line (a strip's trailing tag gives way first), and wear the set model's strip + fraction along the bottom — `progressOf` from the job's own workspace's list and tipus sets (owner, 2026-09-22: "they show the same name… the little lines on the bottom as well")
- ☑ The drawer's plan pane (and its full screen, and the studio's Pan tool) PANS on a left-button mouse drag; a motionless click on the sheet still opens full screen; a finger/pen keeps its native scroll — `mousePan` on the stage in PlanAnnotator (owner, 2026-09-22: "holding the left button on the mouse and dragging, that doesn't work")
- ☑ Every task EDITOR (the drawer's Tasks tab, the Tasks page, the quick-add panel's edit mode) draws the apartment's stages as bubbles through `StagePairPicker` — the task's own stages open picked, a change saves `stageId` + `stageIds`; no plain stage `<select>` remains in any editor (owner, 2026-09-22: "we didn't do the stage fix with the bubbles… across the whole app… Look how bad it is")
- ☑ The drawer's Add Task on a unit reached from ANOTHER workspace (notebook card, unit card, Building Progress cell) opens the task panel in place; the return ticket home is redeemed when the task PANEL closes, never by the Add Task press (owner, 2026-09-22: "add task… just exits the drawer… and takes me back to the job board without doing anything")
- ☑ "Live from site" (`recent-photos`, all three looks) draws every WORKSPACE's pictures and films newest first, live for the open one and through the foreign sync's new `contractorPhotos` listener for the rest; a shot younger than ten minutes wears a "new" ring; every tile is a button opening `MediaViewer` (film: play, sound, full screen) on the board and on the TV (owner, 2026-09-23: "nothing shows up there the second it comes in… from the TV it needs to be touchable")
- ☑ Every reply ends, after the Bottom line, with ONE bold one-liner with emojis saying whether the owner needs to do anything (owner, 2026-09-23)
